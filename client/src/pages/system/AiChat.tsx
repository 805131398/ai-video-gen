...
import {
  ...
  ChevronDown, Sparkles, Image, Video, Music, Star, Layers, ImagePlus, RefreshCw,
} from 'lucide-react';
import { AiToolType, AiToolConfig, ChatConversation, ChatMessage, VideoGenProgress } from '../../types';
...
import VideoParamPanel from '../../components/VideoParamPanel';
...
export default function AiChat() {
  ...
  const isVideoMode = selectedConfig?.toolType === 'video_gen' && !!selectedConfig.extraConfig;

  // 视频模式：初始化参数默认值
  useEffect(() => {
    if (isVideoMode && selectedConfig?.extraConfig) {
  ...
  const startPolling = useCallback((params: {
    ...
    const { config, taskId, convId, assistantMsgId, prompt, paramValues, configId } = params;
    ...
    setVideoProgress({ stage: 'generating', taskId });

    const poll = async () => {
      try {
        const statusResult = await videoPollStatus({
          baseUrl: config.baseUrl,
          apiKey: config.apiKey,
          statusConfig: extraConfig.endpoints.status,
          taskId,
        ...
        console.log('[AiChat:poll]', taskId, JSON.stringify(statusResult, null, 2));

        if (!statusResult.success) {
          console.warn('[AiChat:poll] 请求失败，继续轮询:', statusResult.error);
          return;
        }

        const status = statusResult.status || '';
        console.log(`[AiChat:poll] taskId=${taskId}, status="${status}"`);
        setVideoProgress((prev) => prev ? { ...prev, status } : prev);

        // 检查终态 - 成功
        if (['completed', 'complete', 'succeeded', 'success'].some(s => status.toLowerCase().includes(s))) {
          if (pollTimerRef.current) clearInterval(pollTimerRef.current);
          pollTimerRef.current = null;

          const videoUrl = statusResult.videoUrl || '';
          const thumbnailUrl = statusResult.thumbnailUrl || '';

          console.log(`[AiChat:poll] 任务完成: taskId=${taskId}, videoUrl=${videoUrl}`);

          setVideoProgress({ stage: 'completed', taskId, videoUrl, thumbnailUrl });
          setMessages((prev) => prev.map((m) =>
            m.id === assistantMsgId ? {
              ...m,
              content: videoUrl ? '视频已生成' : '视频生成完成，但未获取到视频地址',
              videoMeta: { ...m.videoMeta, taskId, videoUrl, thumbnailUrl, status: 'completed' },
            } : m
          ));
          await saveChatMessage({
            ...
            content: videoUrl ? '视频已生成' : '视频生成完成',
            modelName: config.modelName || config.name,
            createdAt: new Date().toISOString(),
            videoMeta: { prompt, params: { ...paramValues }, taskId, videoUrl, thumbnailUrl, status: 'completed', modelConfigId: configId },
          });
          setUploadedImagePath(null);
        }
        // 检查终态 - 失败
        else if (['failed', 'error', 'cancelled'].some(s => status.toLowerCase().includes(s))) {
          if (pollTimerRef.current) clearInterval(pollTimerRef.current);
          pollTimerRef.current = null;

          const rawInfo = statusResult.rawResponse ? JSON.stringify(statusResult.rawResponse).slice(0, 300) : '';
          console.error(`[AiChat:poll] 任务失败: taskId=${taskId}, status="${status}", raw=${rawInfo}`);

          setVideoProgress({ stage: 'failed', taskId, error: `任务状态: ${status}` });
          const failContent = `❌ 视频生成失败 (${status})\n\n详情: ${rawInfo || '无'}`;
          setMessages((prev) => prev.map((m) =>
            m.id === assistantMsgId ? {
              ...m,
              content: failContent,
              videoMeta: { ...m.videoMeta, taskId, status: 'failed' },
            } : m
          ));
          await saveChatMessage({
            id: assistantMsgId,
            ...
            modelName: config.modelName || config.name,
            createdAt: new Date().toISOString(),
            videoMeta: { prompt, params: { ...paramValues }, taskId, status: 'failed', modelConfigId: configId },
          });
        }
      } catch (err) {
  ...
  // 恢复未完成的视频生成轮询
  useEffect(() => {
    if (allConfigs.length === 0 || messages.length === 0) return;
    // 已经有轮询在跑就不重复启动
    if (pollTimerRef.current) return;

    const generatingMsg = messages.find(
      (m) => m.role === 'assistant' && m.videoMeta?.status === 'generating' && m.videoMeta?.taskId
    );
    if (!generatingMsg || !generatingMsg.videoMeta) return;

    const { taskId, modelConfigId, prompt, params } = generatingMsg.videoMeta;
    if (!taskId) return;

    const config = allConfigs.find((c) => c.id === modelConfigId);
    if (!config || !config.extraConfig) {
      console.warn('[AiChat:resume] 找不到模型配置，无法恢复轮询:', modelConfigId);
      return;
    }

    console.log(`[AiChat:resume] 恢复视频轮询: taskId=${taskId}, msgId=${generatingMsg.id}`);
    startPolling({
      config,
      taskId,
      convId: generatingMsg.conversationId,
      assistantMsgId: generatingMsg.id,
      prompt: prompt || '',
      paramValues: params || {},
      configId: modelConfigId || '',
    });
  }, [allConfigs, messages, startPolling]);
  ...
  const handleVideoSend = async () => {
    const text = input.trim();
    if (!text || isStreaming || !selectedConfig || !selectedConfig.extraConfig) return;
    if (videoProgress && ['uploading', 'submitting', 'generating'].includes(videoProgress.stage)) return;

    const extraConfig = selectedConfig.extraConfig;
    let convId = activeConvId;
    ...
    // 创建 assistant 占位消息
    const assistantMsgId = crypto.randomUUID();
    const videoMeta = { prompt: text, params: { ...videoParamValues }, status: 'generating', modelConfigId: selectedConfigId };
    const assistantMsg: ChatMessage = {
      id: assistantMsgId,
      conversationId: convId,
      role: 'assistant',
    ...
    try {
      ...
      if (uploadedImagePath) {
        setVideoProgress({ stage: 'uploading' });
        const uploadResult = await videoUpload({
          baseUrl: selectedConfig.baseUrl,
          apiKey: selectedConfig.apiKey,
          uploadConfig: extraConfig.endpoints.upload,
          filePath: uploadedImagePath,
        });
        if (!uploadResult.success) {
          setVideoProgress({ stage: 'failed', error: uploadResult.error || '图片上传失败' });
          setMessages((prev) => prev.map((m) =>
            m.id === assistantMsgId ? { ...m, content: `❌ 图片上传失败: ${uploadResult.error}`, videoMeta: { ...m.videoMeta, status: 'failed' } } : m
          ));
          await saveChatMessage({ ...assistantMsg, content: `❌ 图片上传失败: ${uploadResult.error}`, videoMeta: { ...videoMeta, status: 'failed' } });
          return;
        }
      ...
      // Step 2: 提交生成任务
      setVideoProgress({ stage: 'submitting' });
      const genResult = await videoGenerate({
        baseUrl: selectedConfig.baseUrl,
        apiKey: selectedConfig.apiKey,
        generateConfig: extraConfig.endpoints.generate,
        prompt: text,
        imageUrl,
        paramValues: videoParamValues,
        conversationId: convId,
        modelConfigId: selectedConfigId,
        modelName: selectedConfig.modelName || undefined,
      });

      if (!genResult.success || !genResult.taskId) {
        const rawInfo = genResult.rawResponse ? JSON.stringify(genResult.rawResponse).slice(0, 300) : '';
        console.error('[AiChat:videoSend] 任务提交失败:', genResult.error, rawInfo);
        setVideoProgress({ stage: 'failed', error: genResult.error || '任务提交失败' });
        const failContent = `❌ 任务提交失败: ${genResult.error}\n\n详情: ${rawInfo || '无'}`;
        setMessages((prev) => prev.map((m) =>
          m.id === assistantMsgId ? { ...m, content: failContent, videoMeta: { ...m.videoMeta, status: 'failed' } } : m
        ));
        await saveChatMessage({ ...assistantMsg, content: failContent, videoMeta: { ...videoMeta, status: 'failed' } });
        return;
      }

      const taskId = genResult.taskId;
      console.log(`[AiChat:videoSend] 任务已提交: taskId=${taskId}`, genResult.rawResponse);

      // 立即持久化 generating 状态到数据库，确保页面切换后可恢复
      const generatingMeta = { ...videoMeta, taskId, status: 'generating' };
      setMessages((prev) => prev.map((m) =>
        m.id === assistantMsgId ? { ...m, videoMeta: generatingMeta } : m
      ));
      await saveChatMessage({ ...assistantMsg, videoMeta: generatingMeta });
      ...
      setVideoProgress({ stage: 'failed', error: error.message });
      setMessages((prev) => prev.map((m) =>
        m.id === assistantMsgId ? { ...m, content: `❌ 错误: ${error.message}`, videoMeta: { ...m.videoMeta, status: 'failed' } } : m
      ));
      await saveChatMessage({ ...assistantMsg, content: `❌ 错误: ${error.message}`, videoMeta: { ...assistantMsg.videoMeta, status: 'failed' } });
    }
  };

  // 取消视频生成轮询
  const handleCancelVideoGen = () => {
    if (pollTimerRef.current) {
      clearInterval(pollTimerRef.current);
      pollTimerRef.current = null;
    }
    setVideoProgress((prev) => prev ? { ...prev, stage: 'failed', error: '已取消' } : prev);
  };

  // 重新获取视频地址（用于旧数据 videoUrl 为空的情况）
  const [refetchingMsgId, setRefetchingMsgId] = useState<string | null>(null);
  const handleRefetchVideo = async (msg: ChatMessage) => {
    const meta = msg.videoMeta;
    if (!meta?.taskId) return;

    // 找到对应的模型配置
    const configId = meta.modelConfigId
      || conversations.find((c) => c.id === msg.conversationId)?.modelConfigId;
    const config = allConfigs.find((c) => c.id === configId);
    if (!config?.extraConfig) {
      showToast('找不到对应的模型配置，无法重新获取', 'error');
      return;
    }

    setRefetchingMsgId(msg.id);
    try {
      const statusResult = await videoPollStatus({
        baseUrl: config.baseUrl,
        apiKey: config.apiKey,
      ...
      console.log('[AiChat:refetch]', meta.taskId, statusResult);

      if (!statusResult.success) {
        showToast(`查询失败: ${statusResult.error}`, 'error');
        return;
      }
      ...
      if (!videoUrl) {
        showToast(`未获取到视频地址 (status: ${statusResult.status || 'unknown'})`, 'error');
        return;
      }

      // 更新内存和数据库
      const updatedMeta = { ...meta, videoUrl, thumbnailUrl, status: 'completed', modelConfigId: configId || undefined };
      setMessages((prev) => prev.map((m) =>
        m.id === msg.id ? { ...m, content: '视频已生成', videoMeta: updatedMeta } : m
      ));
      await saveChatMessage({ ...msg, content: '视频已生成', videoMeta: updatedMeta });
      showToast('视频地址已更新');
    } catch (err: any) {
      console.error('[AiChat:refetch] error:', err);
      showToast(`获取失败: ${err.message}`, 'error');
    } finally {
      setRefetchingMsgId(null);
    }
  };
  ...
  const handleSelectImage = async () => {
    try {
      const filePath = await window.electron.storage.selectFile({
        title: '选择参考图片',
        filters: [
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    if (e.key === 'Enter' && !e.shiftKey) {
      e.preventDefault();
      isVideoMode ? handleVideoSend() : handleSend();
    }
  };
  ...
  const handleSaveResources = async (msg: ChatMessage, customPath?: string) => {
    ...
    // 视频消息：videoUrl 在 videoMeta 里而非 content 中
    if (msg.videoMeta?.videoUrl && !resources.some((r) => r.url === msg.videoMeta!.videoUrl)) {
      resources.push({ url: msg.videoMeta.videoUrl, type: 'video' });
    }
    if (msg.videoMeta?.thumbnailUrl && !resources.some((r) => r.url === msg.videoMeta!.thumbnailUrl)) {
      resources.push({ url: msg.videoMeta.thumbnailUrl, type: 'image' });
    }
    ...
    try {
      ...
      let updatedMeta = msg.videoMeta ? { ...msg.videoMeta } : null;
      let allSuccess = true;
      ...
      for (const resource of resources) {
        ...
        if (result.success && result.localPath) {
          ...
          // 同步更新 videoMeta 中的地址
          if (updatedMeta) {
            if (updatedMeta.videoUrl === resource.url) updatedMeta.videoUrl = localUrl;
            if (updatedMeta.thumbnailUrl === resource.url) updatedMeta.thumbnailUrl = localUrl;
          }
        } else {
          allSuccess = false;
        }
      ...
      if (allSuccess) {
        const updatedMsg = { ...msg, content: updatedContent, videoMeta: updatedMeta };
    ...
    return (
      <div key={label}>
        ...
        {convs.map((conv) => (
          ...
          >
            {(() => {
              const isGenerating = activeConvId === conv.id && videoProgress && ['uploading', 'submitting', 'generating'].includes(videoProgress.stage);
              if (isGenerating) return <Loader2 className="w-4 h-4 flex-shrink-0 text-blue-500 animate-spin" />;
              const TypeIcon = TOOL_TYPE_ICONS[conv.toolType] || MessageSquare;
              return <TypeIcon className="w-4 h-4 flex-shrink-0" />;
            })()}
  ...
  return (
    <div className="h-full flex">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        {/* 视频生成进度条 */}
        {isVideoMode && videoProgress && videoProgress.stage !== 'completed' && (
          <VideoProgressBar progress={videoProgress} onCancel={handleCancelVideoGen} />
        )}
          ...
          {messages.map((msg) => {
            ...
            >
              ...
              <div className={`max-w-[70%] ${isAssistant ? '' : ''}`}>
                ...
                >
                  {isAssistant ? (
                    msg.videoMeta?.videoUrl ? (
                      <VideoMessageBubble
                        videoUrl={msg.videoMeta.videoUrl}
                        thumbnailUrl={msg.videoMeta.thumbnailUrl}
                        prompt={msg.videoMeta.prompt}
                        params={msg.videoMeta.params}
                        onSave={() => handleSaveResources(msg)}
                        onSaveAs={() => handleSaveAs(msg)}
                      />
                    ) : msg.videoMeta?.taskId && msg.videoMeta?.status === 'completed' && !msg.videoMeta?.videoUrl ? (
                      <div className="space-y-2">
                        <p className="text-slate-500">视频已生成，但未获取到地址</p>
                        <button
                          onClick={() => handleRefetchVideo(msg)}
                          disabled={refetchingMsgId === msg.id}
                          className="flex items-center gap-1.5 px-3 py-1.5 text-xs bg-blue-50 text-blue-600 rounded-lg hover:bg-blue-100 transition-colors cursor-pointer disabled:opacity-50"
                        >
                          {refetchingMsgId === msg.id
                            ? <Loader2 className="w-3.5 h-3.5 animate-spin" />
                            : <RefreshCw className="w-3.5 h-3.5" />
                          }
                          <span>{refetchingMsgId === msg.id ? '获取中...' : '重新获取视频'}</span>
                        </button>
                      </div>
                    ) : hasContent ? (
        ...
        <div className="border-t border-slate-200 bg-white">
          {/* 视频模式参数面板 */}
          {isVideoMode && selectedConfig?.extraConfig && (
            <VideoParamPanel
              params={selectedConfig.extraConfig.endpoints.generate.params}
              values={videoParamValues}
              onChange={setVideoParamValues}
              disabled={!!videoProgress && ['uploading', 'submitting', 'generating'].includes(videoProgress.stage)}
            />
          )}
          <div className="px-6 py-4">
            {/* 已选图片提示 */}
            {isVideoMode && uploadedImagePath && (
              <div className="flex items-center gap-2 mb-2 px-3 py-1.5 bg-blue-50 rounded-lg text-xs text-blue-600 border border-blue-200">
                <Image className="w-3.5 h-3.5" />
                <span className="truncate flex-1">{uploadedImagePath}</span>
                <button onClick={() => setUploadedImagePath(null)} className="p-0.5 hover:text-blue-800 cursor-pointer">
                  <X className="w-3 h-3" />
                </button>
              </div>
            )}
            <div className="group/input rounded-2xl border border-slate-200 bg-slate-50/50 transition-all focus-within:border-blue-400 focus-within:bg-white focus-within:shadow-[0_0_0_3px_rgba(59,130,246,0.1)]">
              <textarea
                ref={inputRef}
                value={input}
                onChange={(e) => setInput(e.target.value)}
                onKeyDown={handleKeyDown}
                placeholder={isVideoMode ? '输入视频描述 prompt...' : (selectedConfig ? '输入消息...' : '请先选择模型')}
                disabled={!selectedConfig || isStreaming || (!!videoProgress && ['uploading', 'submitting', 'generating'].includes(videoProgress.stage))}
                rows={1}
                className="w-full resize-none bg-transparent px-4 pt-3 pb-1 text-sm outline-none placeholder:text-slate-400 disabled:text-slate-400 max-h-32 overflow-y-auto"
              ...
              <div className="flex items-center justify-between px-3 pb-2.5">
                <div className="flex items-center gap-1">
                  ...
                  {isVideoMode && (
                    <button
                      ...
                      disabled={!!videoProgress && ['uploading', 'submitting', 'generating'].includes(videoProgress.stage)}
                      className="flex items-center justify-center w-8 h-8 text-slate-400 rounded-lg hover:bg-slate-100 hover:text-blue-600 disabled:opacity-50 transition-colors cursor-pointer"
                      title="上传参考图片（图生视频）"
                    >
                ...
                <button
                  onClick={isVideoMode ? handleVideoSend : handleSend}
                  disabled={!input.trim() || isStreaming || !selectedConfig || (!!videoProgress && ['uploading', 'submitting', 'generating'].includes(videoProgress.stage))}
                  className="flex items-center justify-center w-8 h-8 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors cursor-pointer"
                >
                  {(isStreaming || (videoProgress && ['uploading', 'submitting', 'generating'].includes(videoProgress.stage)))
                    ? <Loader2 className="w-4 h-4 animate-spin" />
                    : isVideoMode ? <Video className="w-4 h-4" /> : <Send className="w-4 h-4" />
                  }
                </button>
              </div>
            </div>
