...
import {
  ...
  ChevronLeft, ChevronRight, X, Trash2, Search, Copy, Check,
    ...
    <div className="h-full flex flex-col">
        ...
        <div className="flex bg-slate-100 rounded-lg p-0.5">
          {TIME_RANGES.map((r) => (
            <button
              ...
              className={`px-3 py-1 text-xs rounded-md transition-colors cursor-pointer ${timeRange === r.value
      ...
      <div className="flex-1 flex overflow-hidden px-6 pb-4">
        <div className={`flex-1 flex flex-col bg-white rounded-xl border border-slate-200 overflow-hidden ${selectedLog ? 'mr-4' : ''}`}>
          {/* 表头 */}
          <div className="grid grid-cols-[140px_80px_1fr_70px_80px_80px_1fr_70px] gap-2 px-4 py-2.5 bg-slate-50 border-b border-slate-200 text-xs font-medium text-slate-500">
            <div>时间</div>
            <div>类型</div>
            <div>模型</div>
            <div>状态</div>
              ...
              logs.map((log) => {
                ...
                return (
                  <div
                    ...
                    className={`grid grid-cols-[140px_80px_1fr_70px_80px_80px_1fr_70px] gap-2 px-4 py-2.5 border-b border-slate-100 text-sm cursor-pointer transition-colors hover:bg-slate-50 ${selectedLog?.id === log.id ? 'bg-blue-50' : ''
                      }`}
                  >
                    ...
                    <div className="flex justify-end items-center gap-1">
                      <button
                        onClick={async (e) => {
                          ...
                          const lines = [
                            `【基础信息】`,
                            `操作类型: ${TOOL_TYPE_MAP[log.toolType as AiToolType]?.label || log.toolType}`,
                            `模型: ${log.modelName || '-'}`,
                            `状态: ${log.status === 'success' ? '成功' : '失败'}`,
                            `耗时: ${formatDuration(log.durationMs)}`,
                            `时间: ${new Date(log.createdAt).toLocaleString('zh-CN')}`,
                            `会话 ID: ${log.conversationId || '-'}`,
                            ``,
                            `【Token 消耗】`,
                            `Prompt Tokens: ${log.promptTokens || 0}`,
                            `Completion Tokens: ${log.completionTokens || 0}`,
                            `总消耗 Tokens: ${log.totalTokens || 0}`,
                            ``,
                            `【API 参数】`,
                            `Base URL: ${log.baseUrl || '-'}`,
                        ...
                        title="复制详情"
                      >
                        {copiedId === log.id ? <Check className="w-3.5 h-3.5 text-green-500" /> : <Copy className="w-3.5 h-3.5" />}
                      </button>
                      <button
                ...
                {Array.from({ length: Math.min(5, totalPages) }, (_, i) => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-7 h-7 text-xs rounded cursor-pointer ${page === pageNum
